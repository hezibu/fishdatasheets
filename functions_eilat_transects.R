...
create_transects_meta_sheet <- function(input_data){
...
get_transects_surveyors_data <- function(input_data) {
  ...
  surveyors <- input_data %>%
    ...
    select(`First Observer`,`Second Observer`,sheet_name,sites,emails)
  return(surveyors)
}
...
create_transects_observer_tables <- function(surveyors_data,expedition_name,folder_name) {
  pmap(.l = list(surveyors_data$`First Observer`,surveyors_data$`Second Observer`,
                 surveyors_data$sheet_name,
                 surveyors_data$sites,
                 surveyors_data$emails), 
       .f = function(observer1,observer2,sheet_name,sites,emails){
         spreadsheet <- googledrive::drive_cp(file = "~/Data Sheets/Skeleton Folder/Eilat Skeleton - Transects",
                                              path = str_glue("~/Data Sheets/{expedition_name}/{folder_name}/"),
                                              name = sheet_name)
         for (i in length(sites):1){
           for (transect_letter in LETTERS[3:1]){
             googlesheets4::sheet_copy(from_ss = spreadsheet,from_sheet = "Observer Table - Cryptic - MASTER",to_ss = spreadsheet,
                                       to_sheet = str_glue("Dive {i} - Transect {transect_letter} - CRYPTIC"))
             googlesheets4::range_write(ss = spreadsheet,sheet = str_glue("Dive {i} - Transect {transect_letter} - CRYPTIC"),
             ...
             googlesheets4::sheet_copy(from_ss = spreadsheet,from_sheet = "Observer Table - MASTER",to_ss = spreadsheet,
                                       to_sheet = str_glue("Dive {i} - Transect {transect_letter} - TRANSIENTS"))
             googlesheets4::range_write(ss = spreadsheet,sheet = str_glue("Dive {i} - Transect {transect_letter} - TRANSIENTS"),
                                        col_names = F, range = "B2:B3",
                                        data = as.data.frame(c(observer1,observer2)),reformat = F)
           } 
         }
